...
class WelcomeSignup extends StatefulWidget {
  ...
  const WelcomeSignup({super.key, this.onBackPressed, this.onLoginPressed});
                  ...
                  Consumer<AuthProviderV2>(
                    builder: (context, auth, _) => ElevatedButton(
                      onPressed: auth.isLoading || !_rememberMe
                          ? null
                          : () async {
                              // Prevent double-tap by checking isLoading again
                              ...
                              if (result != null && mounted) {
                                ...
                                final createdUserId = result.user.userId;

                                // Navigate to verify email screen and pass createdUserId
                                Navigator.of(context).pushReplacement(
                                  MaterialPageRoute(
                                    builder: (_) => WelcomeVerifyEmail(
                                      initialEmail: email,
                                      // forward the created user id for later profile setup
